...
export default function LoginPage({ onNavigate }) {
  ...
  // ── BUG 1: handleLogin() usa login() del contexto, que llama
  //    al endpoint incorrecto /auth/iniciar-sesion (no existe).
  //    El botón de acceso directo usa loginDirect() que sí funciona.
  const handleLogin = async () => {
    if (!email || !password) { showToast('Completa todos los campos.', 'error'); return; }
    setLoading(true);
    try {
      await login(email, password); // BUG 1: llama al endpoint equivocado
